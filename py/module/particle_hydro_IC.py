...
IC_PLOT_AT_REDISTRIBUTION = (
    True  # create and store a plot of the current situation before redistributing?
)
...
def generate_IC_for_given_density(
    rho_anal, nx, ndim, eta, x=None, m=None, kernel="cubic spline", periodic=True
):
    """
    ...
    MID = 1.0 / npart ** (1.0 / ndim)  # mean interparticle distance
    delta_r_norm = IC_DELTA_INIT * MID
    delta_r_min = IC_DELTA_MIN * MID

    if IC_PLOT_AT_REDISTRIBUTION:
        # drop a first plot
        h, rho, _, ncells_proper, _ = compute_smoothing_lengths(
            x, m, eta, kernel=kernel, ndim=ndim, periodic=periodic, ncells=None
        )
    ...
    while True:
        ...
        if (
            max_deviation < IC_DISPLACEMENT_THRESHOLD
    ...
    print("Happy?")
    h, rho, _, ncells_proper, _ = compute_smoothing_lengths(
        x, m, eta, kernel=kernel, ndim=ndim, periodic=periodic, ncells=ncells_proper
    )
...
def redistribute_particles(
    x, h, rho, rhoA, iteration, kernel="cubic spline", ndim=2, periodic=True
):
    """
    ...
    to_move = int(
        npart
        * IC_REDISTRIBUTE_FRACTION
        * (1.0 - (iteration / IC_NO_REDISTRIBUTION_AFTER) ** 3)
    )
    ...
    if periodic:
        ...
        x[x > 1.0] = 1.0 - np.random.uniform(x[x > 1.0].shape) * 1.0 / npart ** (
            1.0 / ndim
        )
        x[x < 0.0] = 0.0 + np.random.uniform(x[x < 0.0].shape) * 1.0 / npart ** (
            1.0 / ndim
        )
