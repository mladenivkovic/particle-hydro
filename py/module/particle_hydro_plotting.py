...
def plot_1D(x, rho, u, p, t=None, kwargs={}):
    ...
    im1 = ax1.scatter(x, rho, **kwargs,)
    ax1.set_ylabel("density")

    ax2 = fig.add_subplot(1, 3, 2)
    im2 = ax2.scatter(x, u, **kwargs,)
    ax2.set_ylabel("velocity")

    ax3 = fig.add_subplot(1, 3, 3)
    im3 = ax3.scatter(x, p, **kwargs,)
...
def plot_2D(x, rho, u, p, t=None, nx=100, kwargs={}):
    ...
    rhoH, _, _ = np.histogram2d(
        x[:, 1], x[:, 0], bins=nx, range=[[0, 1], [0, 1]], weights=rho
    )
    uxH, _, _ = np.histogram2d(
        x[:, 1], x[:, 0], bins=nx, range=[[0, 1], [0, 1]], weights=u[:, 0]
    )
    uyH, _, _ = np.histogram2d(
        x[:, 1], x[:, 0], bins=nx, range=[[0, 1], [0, 1]], weights=u[:, 1]
    )
    pH, _, _ = np.histogram2d(
        x[:, 1], x[:, 0], bins=nx, range=[[0, 1], [0, 1]], weights=p
    )

    fig = plt.figure(figsize=(21, 5))

    ax1 = fig.add_subplot(1, 4, 1)
    im1 = ax1.imshow(rhoH, origin="lower", extent=(0, 1, 0, 1), **kwargs,)
    ax1.set_title("density")

    ax2 = fig.add_subplot(1, 4, 2)
    im2 = ax2.imshow(uxH, origin="lower", extent=(0, 1, 0, 1), **kwargs,)
    ax2.set_title("velocity in x direction")

    ax3 = fig.add_subplot(1, 4, 3)
    im3 = ax3.imshow(uyH, origin="lower", extent=(0, 1, 0, 1), **kwargs,)
    ax3.set_title("velocity in y direction")

    ax4 = fig.add_subplot(1, 4, 4)
    im4 = ax4.imshow(pH, origin="lower", extent=(0, 1, 0, 1), **kwargs,)
...
def plot_2D_profile(x, rho, u, p, t=None, nx=100, kwargs={}):
    ...
    rhoM, bin_edges, binnumber = stats.binned_statistic(
        X, rho, bins=nx, statistic="mean", range=(0, 1)
    )
    rhoErr, bin_edges, binnumber = stats.binned_statistic(
        X, rho, bins=nx, statistic="std", range=(0, 1)
    )
    uxM, bin_edges, binnumber = stats.binned_statistic(
        X, u[:, 0], bins=nx, statistic="mean", range=(0, 1)
    )
    uxErr, bin_edges, binnumber = stats.binned_statistic(
        X, u[:, 0], bins=nx, statistic="std", range=(0, 1)
    )
    uyM, bin_edges, binnumber = stats.binned_statistic(
        X, u[:, 1], bins=nx, statistic="mean", range=(0, 1)
    )
    uyErr, bin_edges, binnumber = stats.binned_statistic(
        X, u[:, 1], bins=nx, statistic="std", range=(0, 1)
    )
    pM, bin_edges, binnumber = stats.binned_statistic(
        X, p, bins=nx, statistic="mean", range=(0, 1)
    )
    pErr, bin_edges, binnumber = stats.binned_statistic(
        X, p, bins=nx, statistic="std", range=(0, 1)
    )
    ...
    ax1 = fig.add_subplot(1, 4, 1)
    im1 = ax1.errorbar(X, rhoM, yerr=rhoErr, **kwargs,)
    ax1.set_title("density")

    ax2 = fig.add_subplot(1, 4, 2)
    im2 = ax2.errorbar(X, uxM, yerr=uxErr, **kwargs,)
    ax2.set_title("velocity in x direction")

    ax3 = fig.add_subplot(1, 4, 3)
    im3 = ax3.errorbar(X, uyM, yerr=uyErr, **kwargs,)
    ax3.set_title("velocity in y direction")

    ax4 = fig.add_subplot(1, 4, 4)
    im4 = ax4.errorbar(X, pM, yerr=pErr, **kwargs,)
    ax4.set_title("pressure")
...
def scatter_2D(x, rho, u, p, t=None, nx=100, kwargs={}):
    ...
    ax1 = fig.add_subplot(1, 4, 1)
    im1 = ax1.scatter(X, Y, c=rho, alpha=alpha, **kwargs,)
    ax1.set_title("density")

    ax2 = fig.add_subplot(1, 4, 2)
    im2 = ax2.scatter(X, Y, c=u[:, 0], alpha=alpha, **kwargs,)
    ax2.set_title("velocity in x direction")

    ax3 = fig.add_subplot(1, 4, 3)
    im3 = ax3.scatter(X, Y, c=u[:, 1], alpha=alpha, **kwargs,)
    ax3.set_title("velocity in y direction")

    ax4 = fig.add_subplot(1, 4, 4)
    im4 = ax4.scatter(X, Y, c=p, alpha=alpha, **kwargs,)
    ax4.set_title("pressure")
