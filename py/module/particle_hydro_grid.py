...
def compute_smoothing_lengths(
    x, m, eta, kernel="cubic spline", ndim=2, periodic=True, ncells=None
):
    """
    ...
    HMAX = 0.5
    HMIN = nparttot ** (-1.0 / ndim) * 2  # take at least 2 interparticle distances
        ...
        for p in c.parts:
            ...
            while True:
                ...
                if abs(Hinew - Hi) < EPSILON * Hi:
                    ...
                    neighbours[p] = copy.copy(
                        nbrsort[1:i]
                    )  # index 0 is particle itself, skip that
                    break
                else:
                    Hiprev = Hi
                    Hi = Hinew
                ...
                if niter == ITER_MAX:
                    ...
                    print(
                        "Reached max number of iterations for smoothing length. ",
                    ...
                    if ndim == 1:
                        ...
                        for i in range(ncells):
                            ...
                            ax1.plot(
                                [0, 1], [(i + 1) / ncells, (i + 1) / ncells], c="k"
                            )
                        # draw smoothing length
                        ax1.errorbar(
                            x=x[p, 0], y=x[p, 1], xerr=[Hinew], yerr=[Hinew], c="red"
                        )
                        # draw particles
...
def build_grid(
    x, m, eta, kernel="cubic spline", ndim=2, periodic=True, verbose=True, ncells=None
):
    """
    ...
    while repeat:
        ...
        for c in grid:
            ...
            if nbparts <= min_neighbour_fact_for_cells * nngb:
                ncells -= 1
                print(
                    "got too few neighbours in cell",
                    c.ind,
                    ": particles found:",
                    nbparts,
                    "want:",
